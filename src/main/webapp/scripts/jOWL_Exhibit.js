...
(function () {
  ...
  jOWL.Exhibit = function (owlclass, options) {
    ...
    if (!owlclass) {
      throw "no owl:Class specified for Exhibit.jOWL()";
    }
    ...
    function configureDom() {
      ...
      {
        if (jOWL.Exhibit.defaults.facet) {
          facetpanel =
        ...
        if (jOWL.Exhibit.defaults.lens) {
          ...
          if (!lbl.length) {
            throw 'Lens Template: no element found with attribute class="' + jOWL.Exhibit.defaults.css.label + '"';
          }
          ...
          if (type) {
            type.attr("ex:content", ".owlClass");
          }
        }
        jowl.sourceof().each(function (item, i) {
          if (facetpanel) {
            facetpanel.append($("<div ex:role='facet' ex:expression='." + item.property.name + "' ex:facetLabel='" + item.property.name + "'/>"));
          }
          if (lenspanel) {
            configureCriteria(lensclone.clone(true), item.property.name).appendTo(lenswrapper);
          }
        });
        if (lenspanel) {
          lenspanel.show();
        ...
        onComplete: function (indarr) {
          ...
          indarr.jOWLArray('?x').each(function () {
            ...
            this.valueRestrictions(true).each(function () {
              if (!entry[this.property.URI]) {
                entry[this.property.URI] = [];
              }
          ...
          if (results.length) {
            jOWL.Exhibit.exhibit.getDatabase().loadData({items: results});
          }
          if (jOWL.Exhibit.defaults.status) {
            jOWL.Exhibit.defaults.status.hide();
          }
    ...
    function complete() {
      configureDom();
      if (jOWL.Exhibit.defaults.status) {
        $('.msg', jOWL.Exhibit.defaults.status).text("Reasoning over Individuals");
      }
      setTimeout(function () {
        loadJSON(jowl);
      }, 5);
