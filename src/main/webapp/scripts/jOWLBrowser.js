...
function showOverviewResults(results, list, widget) {
  ...
  jOWL.throttle(results, {
    ...
    modify: function (item) {
      ...
      $el.click(function () {
        if (widget && widget.propertyChange) {
          widget.propertyChange(jOWL(this.title));
        }
      });
      if (maxcount > 0) {
        if (maxcount != 49)
    ...
    onComplete: function (arr) {
      if (maxcount < 0)
        list.append($("<div class='jowl_link' style='margin-top:5px;'/>").text("... [" + maxcount * -1 + " more]").click(function () {
          hidden.slideDown();
          $(this).hide();
        }));
    },
...
function createConceptWidget() {
  ...
  widget.show = function () {
    ...
    if (configuration.owlClass) {
      concept = jOWL(configuration.owlClass);
    } else {
  ...
  //show hint section
  $('#owlauto').blur(function () {
    $(this).siblings('.info').show();
  }).focus(function () {
    $(this).siblings('.info').fadeOut(1000);
  }).blur();
