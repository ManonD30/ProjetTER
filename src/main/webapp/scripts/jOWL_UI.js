...
(function ($) {
  ...
  jOWL.UI = {
    broadcaster: function () {
      ...
      this.broadcast = function (item) {
        for (var i = 0; i < listeners.length; i++) {
          listeners[i].propertyChange.call(item, item);
        }
      };
    ...
    asBroadcaster: function (ui_elem) {
      ui_elem.broadcaster = jOWL.UI.broadcaster;
      ui_elem.broadcaster();
    },
    defaults: {
  ...
  $.fn.extend({
    ...
    owl_navbar: function (options) {
      ...
      var listnode = $('<span/>').click(function () {
        ...
        if (options.onSelect.call(node, res) === false) {
          return;
        }
        if (res && res.isClass) {
      ...
      this.propertyChange = function (item) {
        if (options.onPropertyChange.call(this, item) === false) {
          return;
        }
        if (item.isClass) {
    ...
    owl_autocomplete: function (options) {
      options = $.extend({
        ...
        formatListItem: function (listitem, type, identifier, termarray) { //formatting of results, can be overridden
          ...
          if (termarray.length) {
            listitem.append($('<div class="terms"/>').text(termarray.join(', '))
      ...
      results.open = function (q, cache) {
        ...
        if (q) {
          if (!cache || results.isEmpty()) {
            results.cache = jOWL.query(q, options);
          } else {
            var newcache = {};
            for (x in results.cache) {
              ...
              var found = false;
              var newentries = [];
              if (x.searchMatch(q) > -1) {
                found = true;
              }
              for (var i = 0; i < entry.length; i++) {
                if (entry[i].term.searchMatch(q) > -1) {
                  found = true;
                  newentries.push(entry[i]);
                }
              }
              if (found) {
                newcache[x] = newentries;
              }
            }
        ...
        for (x in data) {
          if (count < options.limit) {
            ...
            for (var l = 0; l < item.length; l++) {
              var found = false;
              var newterm = item[l].term;
              for (var y = 0; y < terms.length; y++) {
                if (terms[y].toLowerCase() == newterm.toLowerCase()) {
                  found = true;
                }
              }
              if (!found) {
      ...
      setInterval(function () {
        ...
        if (old != newvalue) {
          ...
          old = newvalue;
          if (newvalue.length < options.chars && open) {
            results.close();
            open = false;
          } else if (newvalue.length >= options.chars && newvalue.length > 0) {
            if (cache) {
              cache = longervalue && newvalue.length > options.chars;
            }
    ...
    owl_treeview: function (options) {
      ...
      function TreeModel(owlobject) {
        ...
        if (options.rootThing) {
          traverse(h, tree.root(jOWL("Thing")));
        } else {
          var root = tree.root(h);
          for (var i = 0; i < root.length; i++) {
            traverse(root[i].invParents, root[i]);
            if (!root[i].invParents) {
              leaf(root[i]);
            }
          }
        ...
        fn.node = function (text, isRoot) { //creates a new node
          ...
          if (text) {
            ...
            this.$name.appendTo(this.jnode).click(function () {
              ...
              if (entry && options.onSelect.call(n, entry) === false) {
                return;
              }
              tree.broadcast(entry);
              if (options.isStatic) {
                tree.propertyChange(entry);
              }
              return false;
            });
          }
          ...
          this.add = function (text) {
            ...
            if (!self.wrapper.children().length) {
              toNode();
            } else {
          ...
          function toggle() {
            ...
            self.jnode.swapClass('tvic', 'tvie');
            self.jnode.swapClass('tvilc', 'tvile');
      ...
      tree.propertyChange = function (item) {
        if (item.isClass) {
          var m = new TreeModel(item);
        }
      };
      return tree;
    },
    ...
    owl_propertyLens: function (options) {
      ...
      if (!backlink.length) {
        backlink = $('<div class="jowl_link"/>').addClass(self.options.backlinkClass).text("Back").hide().appendTo(this);
      }
      jOWL.UI.asBroadcaster(this);
      ...
      var action = {
        "rdfs:label": function (item) {
          return [{"rdfs:label": item.label()}];
        },
        "rdf:ID": function (item) {
          return [{"rdf:ID": [item.name, item]}];
        },
        "rdfs:comment": function (item) {
          return $.map(item.description(), function (n) {
            return {"rdfs:comment": n};
          });
        },
        "rdf:type": function (item) {
          if (item.owlClass) {
            return [{"rdf:type": item.owlClass()}];
          }
          return [{"rdf:type": item.type}];
        },
        "term": function (item) {
          return $.map(item.terms(), function (n, i) {
            return {"term": n[0]};
          });
        },
        "rdfs:range": function (item) {
          if (item.range) {
            return [{"rdfs:range": item.range}];
          }
        },
        "rdfs:domain": function (item) {
          if (item.domain) {
            return [{"rdfs:domain": item.domain}];
          }
        },
        ...
        "owl:disjointWith": function (item) {
          ...
          return $.map(
                  ...
                  .filter(function () {
                    return this.nodeName == "owl:disjointWith";
                  }),
                  function (n, i) {
        ...
        "default": function (item) {
          ...
          return $.map(
                  jOWL.Xpath('*', item.jnode).filter(function () {
      ...
      this.propertyChange = function (item) {
        ...
        self.property = item;
        if (backlink.source != item.name) {
          backlink.hide();
        } else {
          backlink.source = false;
        }
        ...
        if (self.options.onUpdate) {
          self.options.onUpdate.call(this, item);
        }
      }; //end property change
    ...
    owl_datafield: function (options) {
      ...
      this.addField = function (property) {
        ...
        var $title = property.bind($("<div/>")).addClass(options.labelClass).appendTo($content).click(function () {
          $content.remove();
          delete pArray[property.URI];
        });
        ...
        if (property.isObjectProperty) {
          ...
          var sp = new jOWL.SPARQL_DL("Type(?t, ?c),PropertyValue(concept, property, ?c)", {concept: self.property, property: property}).execute({
            onComplete: function (obj) {
              if (!obj.results.length) {
                return;
              } //modify to deal with non value results
              obj.sort("?t");
          ...
          var msg = "";
          if (self.messages[property.range]) {
            msg = self.messages[property.range];
          }
      ...
      this.serialize = function () {
        ...
        $('.' + options.selectorClass, self).each(function () {
          ...
          if ($this.is("select")) {
            ...
            if (entry) {
              q.PropertyValue.push([prop, '"' + entry + '"']);
            }
          }
        });
        return q;
  ...
  jOWL.UI.PropertyBox.prototype = {
      ...
      for (x in res.options.onChange) {
        ...
        data.each(function () {
          ...
          if (id != "anonymousOntologyObject") {
            res.options.onChange[$.data(node, 'data-jowl')].call(node, item, jOWL(id), res);
          }
        });
      }
    },
    clear: function () {
      ...
      if (!prev.length) {
        prev = this.valuebox.prev('.jowl-template-splitter');
      }
      if (prev.length) {
        prev.remove();
        this.clear(this.valuebox);
      }
    }
  };
  ...
  jOWL.UI.Template = function (arr, jqel, splitter) {
    ...
    function bindObject(value, jnode) {
      ...
      if (typeof value == 'string') {
        ...
        if (value.length == 2) {
          value[1].bind(jnode).text(value[0]);
          bound = true;
        }
      } else if (value.bind) {
        value.bind(jnode);
